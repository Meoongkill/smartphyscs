...
class PsikologController extends Controller
{
    ...
    public function show($id)
    {
        ...
        foreach ($test_results as $test_result) {
            ...
            // Sum all dimension scores for bot (AI prediction)
            if ($test_result->is_predicted) {
                $score_bot = ($test_result->score_bot_integritas ?? 0) +
                    ($test_result->score_bot_kerja_sama ?? 0) +
                    ($test_result->score_bot_komunikasi ?? 0) +
                    ($test_result->score_bot_orientasi_pada_hasil ?? 0) +
                    ($test_result->score_bot_pelayanan_publik ?? 0) +
                    ($test_result->score_bot_pengembangan_diri_dan_orang_lain ?? 0) +
                    ($test_result->score_bot_mengelola_perubahan ?? 0) +
                    ($test_result->score_bot_pengambilan_keputusan ?? 0) +
                    ($test_result->score_bot_perekat_bangsa ?? 0);

                $score_psikolog = ($test_result->score_human_integritas ?? 0) +
                    ($test_result->score_human_kerja_sama ?? 0) +
                    ($test_result->score_human_komunikasi ?? 0) +
                    ($test_result->score_human_orientasi_pada_hasil ?? 0) +
                    ($test_result->score_human_pelayanan_publik ?? 0) +
                    ($test_result->score_human_pengembangan_diri_dan_orang_lain ?? 0) +
                    ($test_result->score_human_mengelola_perubahan ?? 0) +
                    ($test_result->score_human_pengambilan_keputusan ?? 0) +
                    ($test_result->score_human_perekat_bangsa ?? 0);

                $userScores[$userId]['total_score_bot'] += $score_bot;
                $userScores[$userId]['total_score_psikolog'] += $score_psikolog;
                $userScores[$userId]['is_predicted'] = true;
            }
        }
    ...
    public function result($user_id, $session_id)
    {
        ...
        // Get prediction results (9 dimensions)
        $predictionResult = TestResult::where('user_id', $user_id)
            ->where('session_id', $session_id)
        ...
        if ($predictionResult) {
            ...
            foreach ($dimensionNames as $key => $name) {
                $dimensions[] = [
                    'name' => $name,
                    'score_bot' => $predictionResult->{'score_bot_' . $key},
                    'score_human' => $predictionResult->{'score_human_' . $key},
                    'similarity' => $predictionResult->{'similarity_' . $key},
                ];
            }
        }
        ...
        return Inertia::render('Psikolog/DetailHasilTes', [
            ...
            'is_predicted' => $predictionResult ? true : false,
        ]);
    }
